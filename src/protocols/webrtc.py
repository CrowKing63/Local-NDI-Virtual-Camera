...
class WebRTCAdapter(ProtocolAdapter):
    ...
    async def start(self, port: int, path: str = "") -> None:
        ...
        try:
            self._signaling_server = await asyncio.start_server(
                self._handle_signaling_connection,
                '0.0.0.0',
                port
            )
    ...
    async def _start_mdns(self) -> None:
        ...
        log.info("mDNS service registered: %s at %s:%d",
                 service_name, local_ip, self._config.signaling_port)
    ...
    async def _handle_offer(self, peer_id: str, message: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            pc = RTCPeerConnection(configuration={
                'iceServers': [{'urls': self._config.stun_servers}]
            })
            ...
            async def on_track(track):
                ...
                if track.kind == "video":
                    # Relay the track (this would connect to frame decoder)
                    # For now, just log that we received it
                    log.info("Video track ready for processing")
